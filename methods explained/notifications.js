...
import ScyllaDbMod from "./ScyllaDb (1).js";
import DateTimeMod from "./DateTime (1).js";
import SafeUtilsMod from "./SafeUtils.js";
import ErrorHandlerCjs from "./ErrorHandler.js";
import LoggerCjs from "./Logger Final (1).js";
import crypto from "crypto";

const ScyllaDb   = ScyllaDbMod?.default ?? ScyllaDbMod;
const DateTime   = DateTimeMod?.default ?? DateTimeMod;
const SafeUtils  = SafeUtilsMod?.default ?? SafeUtilsMod;
const ErrorHandler = ErrorHandlerCjs?.default ?? ErrorHandlerCjs;
const Logger     = LoggerCjs?.default ?? LoggerCjs;

export default class NotificationsAccount {
  /* ============================== Constants =============================== */
  ...
  static async delete(userIdRaw, notificationIdRaw) {
    const { userId, notificationId } = SafeUtils.sanitizeValidate({
  ...
  static async markAllRead(userIdRaw, { pageLimit = 50 } = {}) {
    const { userId, limit } = SafeUtils.sanitizeValidate({
      userId: { value: userIdRaw, type: "string", required: true },
  ...
  static async get(userIdRaw, {
    ...
    if (!cleaned.category && cleaned.excludePinnedInAll) {
      ...
      Object.assign(
        ExpressionAttributeValues.M, // already marshalled map; append safely
        ScyllaDb.marshalItem({ ":skprefix": NotificationsAccount.SORT_PREFIX_FOR_NON_PINNED }).M
      );
    }
  ...
  static async hasAny(userIdRaw) {
    const { userId } = SafeUtils.sanitizeValidate({ userId: { value: userIdRaw, type: "string", required: true } });
    const PK = NotificationsAccount.#pkName();
  ...
  static #nowEpochSeconds() {
    try {
      if (DateTime && typeof DateTime.generateRelativeTimestamp === "function") {
        // Fallback to system time: DateTime file may not expose nowEpochSeconds; use Date.now() / 1000.
  ...
  static #pkName() {
    if (!NotificationsAccount.#partitionKeyAttr || !NotificationsAccount.#sortKeyAttr) {
      ...
      NotificationsAccount.#partitionKeyAttr = schema.PK || NotificationsAccount.ATTR_PK;
      NotificationsAccount.#sortKeyAttr      = schema.SK || NotificationsAccount.ATTR_SK;
    }
